...
*   **範本倉庫 (Template Repo) 配置**：
    *   **📂 `links.md`**：預置一個表格，讓學員填入 Replit 部署網址、Lovable 專案網址。
...
#### 任務 2：10 分鐘全棧開發 (Replit Agent)
*   **目標**：體驗從零到一（含資料庫、後端、部署）的一條龍自動化。
*   **Classroom 部署建議**：
    *   **自動化檢核 (Autograding)**：在 GitHub Actions 中撰寫一個簡單的 `curl` 腳本，去 Ping 學員提供的 Replit URL。若回傳 `200 OK`，則自動判定「部署成功」。
...
*   [ ] **工具選型**：學員是否理解 Lovable 適合做精緻前端，而 Replit 適合做快速全棧部署？
